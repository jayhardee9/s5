...
fn general_specific_energy_equation(
    specific_energy: &Variable,
    u: &Variable,
    speed: &Variable,
    distance: &Variable,
) -> Equation {
    Equation::new(specific_energy.into(), ((speed ^ 2) / 2) - (u / distance))
}
...
pub fn run() {
    ...
    let two_body_equations = vec![
        ...
        Equation::new(
            u * e_x,
            (((v ^ 2) - (u / r)) * r_x) - (((r_x * v_x) + (r_y * v_y) + (r_z * v_z)) * v_x),
        ),
        Equation::new(
            u * e_y,
            (((v ^ 2) - (u / r)) * r_y) - (((r_x * v_x) + (r_y * v_y) + (r_z * v_z)) * v_y),
        ),
        Equation::new(
            u * e_z,
            (((v ^ 2) - (u / r)) * r_z) - (((r_x * v_x) + (r_y * v_y) + (r_z * v_z)) * v_z),
        ),
        Equation::new_assignment(p, (h ^ 2) / u),
        Equation::new_assignment(p, a * (1 - (e ^ 2))),
        Equation::new_assignment(
            e,
            (Formula::from(1) + (2 * me * (h ^ 2)) / (u ^ 2)) ^ half.clone(),
        ),
        ...
        Equation::new_assignment(
            tp,
            Formula::from(2.0 * PI) / (u ^ half.clone()) * (a ^ three_halves.clone()),
        ),
    ...
    state.bind_variable(
        r_p,
        &(from_nautical_miles(100) as f64 + EARTH_RADIUS).into(),
    );
    state.bind_variable(
        r_a,
        &(from_nautical_miles(600) as f64 + EARTH_RADIUS).into(),
    );
    state.bind_variable(u, &EARTH_GRAVITATIONAL_PARAMETER.into());
    ...
    println!(
        "{:?} ",
        OrbitType::classify_by_semi_major_axis(state.get_binding(a).expect("a solved").to_f64())
    ...
    println!(
        "{:?} ",
        OrbitType::classify_by_eccentricity(state.get_binding(e).expect("e solved").to_f64())
    );
